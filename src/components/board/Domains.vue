<template>
  <div id="domains">
    ...
    <DomainPile v-for="pile in this.$store.state.board.domains" :key="pile.id" :pile="pile" />
  </div>
</template>
...
export default {
  ...
  methods: {
    addDomainsToBoard(domainsPiles){
      domainsPiles.forEach((pile) => {
        this.$store.commit('addDomainPileToBoard', pile);
      })
    },
    setInitialPiles(){
      const domains = [
